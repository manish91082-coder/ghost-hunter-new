...
contract PolygonForkExecutionProbe {
    ...
    function test_real_aave_flashloan_and_dex_callback_path_rolls_back() public {
        ...
        Phase19Executor executor = new Phase19Executor(AAVE_V3_POOL, QUICKSWAP_V2_ROUTER, UNISWAP_V3_ROUTER);
        Phase19Executor.ExecutionParams memory p; p.asset = NATIVE_USDC; p.tokenMid = WMATIC; p.firstOnQuickSwap = true; p.uniswapFee = uniFee;
        ...
        bytes32 topology = executor.routeTopologyHash(NATIVE_USDC, WMATIC, true, uniFee); p.routeCommitment = executor.routeCommitment(QUOTE_ROUTE_HASH, topology); p.intentHash = keccak256("fork-probe-intent");
