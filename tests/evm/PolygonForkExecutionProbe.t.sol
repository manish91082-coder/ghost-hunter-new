...
interface IQuickSwapRouterForkExec {
    function getAmountsOut(uint256 amountIn, address[] calldata path) external view returns (uint256[] memory amounts);
}

interface IUniswapV3FactoryForkExec {
    function getPool(address tokenA, address tokenB, uint24 fee) external view returns (address pool);
}

interface IUniswapV3QuoterV1ForkExec {
...
/// @notice Polygon fork execution probe using the real Aave V3 pool and real DEX routers.
/// @dev The probe intentionally requires an impossible surplus so the whole flash-loan
///      transaction must roll back. This proves the real callback + swap path reaches the
///      executor settlement guard without treating a reverted fork transaction as profit.
contract PolygonForkExecutionProbe {
    Vm internal constant vm = Vm(address(uint160(uint256(keccak256("hevm cheat code")))));

    uint256 internal constant POLYGON_CHAIN_ID = 137;
    uint256 internal constant LOAN_USDC = 100_000_000; // 100 native USDC (6 decimals)
    ...
    function _findUsableUniswapFee(uint256 amountIn, address tokenIn, address tokenOut) internal returns (uint24 fee, uint256 quote) {
        ...
        for (uint256 i = 0; i < feeTiers.length; i++) {
            ...
            address pool = IUniswapV3FactoryForkExec(UNISWAP_V3_FACTORY).getPool(tokenIn, tokenOut, candidate);
            if (pool == address(0) || pool.code.length == 0) continue;
            try IUniswapV3QuoterV1ForkExec(UNISWAP_V3_QUOTER_V1).quoteExactInputSingle(
                tokenIn,
                tokenOut,
                candidate,
                amountIn,
    ...
    function test_real_aave_flashloan_and_dex_callback_path_rolls_back() public {
        ...
        (uint24 uniFee, uint256 uniQuote) = _findUsableUniswapFee(quickQuote[1], WMATIC, NATIVE_USDC);
        require(uniQuote > 0, "invalid Uniswap quote");

        Phase19Executor executor = new Phase19Executor(
            AAVE_V3_POOL,
            QUICKSWAP_V2_ROUTER,
            UNISWAP_V3_ROUTER
        );
        ...
        p.uniswapFee = uniFee;
        p.amountOutMinFirst = quickQuote[1] * 9 / 10;
        p.amountOutMinSecond = uniQuote * 9 / 10;
        p.minimumSurplus = type(uint256).max / 2;
        p.deadline = block.timestamp + 300;
        p.routeHash = QUOTE_ROUTE_HASH;
        p.topologyHash = executor.routeTopologyHash(NATIVE_USDC, WMATIC, true, uniFee);
        p.intentHash = keccak256("fork-probe-intent");

        uint256 beforeBalance = IERC20Fork(NATIVE_USDC).balanceOf(address(executor));
        require(beforeBalance == 0, "unexpected executor balance");

        vm.expectRevert();
        executor.execute(p, LOAN_USDC);

        require(IERC20Fork(NATIVE_USDC).balanceOf(address(executor)) == 0, "fork rollback left USDC");
