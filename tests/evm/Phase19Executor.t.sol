...
contract Phase19ExecutorTest {
    ...
    MockERC20 internal asset; MockERC20 internal mid; MockAavePool internal pool; MockQuickSwapRouter internal quick; MockUniswapV3Router internal uni; Phase19Executor internal executor;
    ...
    function _params(bytes32 intent) internal view returns (Phase19Executor.ExecutionParams memory p) {
        p.asset = address(asset); p.tokenMid = address(mid); p.firstOnQuickSwap = true; p.uniswapFee = 3000; p.amountOutMinFirst = 110 ether; p.amountOutMinSecond = 106 ether;
        p.minimumSurplus = MIN_SURPLUS; p.deadline = 1_000_600; p.routeHash = QUOTE_ROUTE_HASH;
        p.routeCommitment = executor.routeCommitment(p.routeHash, executor.routeTopologyHash(address(asset), address(mid), true, 3000)); p.intentHash = intent;
    }
    ...
    function test_execute_reverse_route_real_callback_swap_repay_and_settlement() public {
        ...
        p.routeCommitment = executor.routeCommitment(p.routeHash, executor.routeTopologyHash(address(asset), address(mid), false, 3000)); executor.execute(p, LOAN); require(asset.balanceOf(address(executor)) == 5 ether, "reverse surplus"); require(executor.consumedIntent(p.intentHash), "reverse intent not consumed");
    }
    ...
    function test_existing_balance_cannot_subsidize_minimum_surplus() public {
        ...
        p.routeCommitment = executor.routeCommitment(p.routeHash, executor.routeTopologyHash(address(asset), address(mid), true, 3000)); vm.expectRevert(Phase19Executor.MinimumSurplusFailed.selector); executor.execute(p, LOAN);
    }
