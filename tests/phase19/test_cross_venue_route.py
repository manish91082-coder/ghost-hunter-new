...
from phantomx.cross_venue_route import build_quickswap_to_uniswap_route
...
def array_result(values):
    ...
    for value in values: raw.extend(value.to_bytes(32, "big"))
...
class FakeRpc:
    ...
    def call(self, method, params):
        ...
        if method == "eth_call":
            to = params[0]["to"]
            if to == ROUTER: return array_result([1000, 900])
...
class CrossVenueRouteTests(unittest.TestCase):
    def test_both_venues_use_one_shared_block_and_exact_amount_continuity(self):
        rpc = FakeRpc()
