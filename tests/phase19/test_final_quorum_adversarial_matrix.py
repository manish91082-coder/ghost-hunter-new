...
class FinalQuorumAdversarialMatrixTests(unittest.TestCase):
    ...
    def test_recovery_drop_replacement_reorg_require_quorum(self):
        ...
        self._new_fixture()
        replacement = self._obs(ChainObservationState.REPLACED, replacement=REPLACEMENT)
        replacement_q = self._quorum(replacement)
        self._admit(replacement_q)
        replacement_result = persist_quorum_recovery_observation(
            store=self.store, intent=self.intent, quorum_observation=replacement_q, observation=replacement,
            tx_nonce=self.intent.nonce, current_observed_block=105, maximum_age_blocks=5,
            minimum_attesting_providers=2, pending_nonce=self.intent.nonce + 1,
        )
        self.assertEqual(replacement_result.transaction_state, ExecutionState.REPLACED)
        self.assertEqual(replacement_result.nonce_state, NonceStatus.REPLACED)

        self._new_fixture()
        included = self._obs(ChainObservationState.INCLUDED)
        included_q = self._quorum(included)
        self._admit(included_q)
        persist_quorum_chain_observation(
            store=self.store, intent=self.intent, quorum_observation=included_q, observation=included,
            tx_nonce=self.intent.nonce, pending_nonce=self.intent.nonce + 1,
            current_observed_block=105, maximum_age_blocks=5, minimum_attesting_providers=2,
            block_hash=OLD_BLOCK, block_number=7000, canonical_block_hash=OLD_BLOCK,
        )
        reorg = self._obs(ChainObservationState.REORGED)
        reorg_q = self._quorum(reorg)
        self._admit(reorg_q)
        reorg_result = persist_quorum_recovery_observation(
            store=self.store, intent=self.intent, quorum_observation=reorg_q, observation=reorg,
            tx_nonce=self.intent.nonce, current_observed_block=105, maximum_age_blocks=5,
            minimum_attesting_providers=2, block_hash=OLD_BLOCK, block_number=7000, canonical_block_hash=BAD_BLOCK,
        )
        self.assertEqual(reorg_result.transaction_state, ExecutionState.REORGED)
        self.assertEqual(reorg_result.nonce_state, NonceStatus.REORGED)
