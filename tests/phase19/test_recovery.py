...
class RecoveryTests(unittest.TestCase):
    def test_pending_holds_state(self):
        decision = decide(
            current_status=NonceStatus.SUBMITTED,
            active_tx_hash=TX,
            observation=ChainTransactionObservation(TX, 7, ChainTxState.PENDING),
    ...
    def test_missing_transaction_does_not_imply_drop(self):
        decision = decide(
            current_status=NonceStatus.SUBMITTED,
            active_tx_hash=TX,
            observation=ChainTransactionObservation(TX, 7, ChainTxState.NOT_FOUND),
            chain_pending_nonce=7,
        )
        self.assertEqual(decision.action, "HOLD")
    ...
    def test_successful_receipt_marks_included(self):
        observation = ChainTransactionObservation(
            TX, 7, ChainTxState.INCLUDED_SUCCESS,
            block_hash="0xblock", receipt_block_hash="0xblock",
            gas_used=21000, effective_gas_price=100,
        )
        decision = decide(
            current_status=NonceStatus.SUBMITTED,
            active_tx_hash=TX,
            observation=observation,
            chain_pending_nonce=8,
        )
        self.assertEqual(decision.next_status, NonceStatus.INCLUDED)

    def test_revert_is_included_but_not_profit_confirmation(self):
        observation = ChainTransactionObservation(
            TX, 7, ChainTxState.INCLUDED_REVERT,
            block_hash="0xblock", receipt_block_hash="0xblock",
            gas_used=50000, effective_gas_price=100,
        )
        decision = decide(
            current_status=NonceStatus.SUBMITTED,
            active_tx_hash=TX,
    ...
    def test_wrong_hash_cannot_mutate_active_record(self):
        with self.assertRaises(RecoveryError):
            decide(
                current_status=NonceStatus.SUBMITTED,
                active_tx_hash=TX,
                observation=ChainTransactionObservation("0x2222", 7, ChainTxState.PENDING),
                chain_pending_nonce=7,
            )
    ...
    def test_unknown_evidence_holds(self):
        decision = decide(
            current_status=NonceStatus.SUBMITTED,
            active_tx_hash=TX,
            observation=ChainTransactionObservation(TX, 7, ChainTxState.UNKNOWN),
            chain_pending_nonce=7,
        )
        self.assertEqual(decision.action, "HOLD")
