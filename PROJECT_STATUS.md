...
- Latest implementation commit: `e5019bcd295366605ea8476052694a269dbe0392`
...
- In-flight recovery regression tests: `e5019bcd295366605ea8476052694a269dbe0392`
...
Chain observation now accepts `SUBMISSION_IN_FLIGHT` and can resolve it to `PENDING`, `INCLUDED`, or `PROFIT_FAILED` on explicit chain evidence. Dedicated recovery accepts `DROPPED`, `REPLACED`, and `REORGED` evidence from the same state without reopening `SIGNED`.

Startup recovery audit now treats `SUBMISSION_IN_FLIGHT` with a SIGNED/hash-free nonce lifecycle as internally consistent, preserving the crash-safe fence across process restart.
...
The new recovery integration commits are now on top of that certified fence. Fresh certification for those newer commits is required before treating the recovery layer as certified.
...
4. Production-like startup/recovery and settlement reorg evidence, including certification of `SUBMISSION_IN_FLIGHT` chain resolution on the latest recovery tree.
...
Atomic task completed: obtain fresh durable-fence certification, then integrate `SUBMISSION_IN_FLIGHT` into chain observation, dedicated recovery, and startup restart audit.

Implementation:
- `0b1acae...` triggered fresh durable-fence certification
- Run `#408` GREEN
...
- `e5019bcd...` adds adversarial restart/recovery regression coverage

The recovery layer itself is now implemented but remains uncertified by a fresh full workflow run.

## 10. NEXT ATOMIC ACTION
Trigger and inspect fresh full Phase-19 certification for `e5019bcd...`. On GREEN, perform the next adversarial gate: prove a recovered `PENDING/INCLUDED/REORGED` outcome cannot bypass settlement/reconciliation controls, and verify no recovery path can create a second submission authority for the same immutable transaction.
