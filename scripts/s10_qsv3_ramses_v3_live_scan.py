...
def classify_s10_tile_failure(exc: BaseException) -> str:
...
def summarize_s10_coverage(tiles: list[dict[str, Any]], expected_tile_count: int, pair_universe_status: str) -> dict[str, Any]:
    completed = sum(
        1 for item in tiles
...
def _scan_rpc(rpc: Any, provider_label: str) -> dict[str, Any]:
        ...
        for pair_name, token_b in active_pairs:
            try:
                ...
                for amount in amounts:
                    try:
                        forward.append(build_quickswap_v3_to_ramses_v3_route(
                            rpc, quickswap, ramses, amount_in=amount, token_a=USDC_E, token_b=token_b,
                            ramses_tick_spacing=tick_spacing, block=context))
                    except Exception as exc:
                        failures.append({"direction":"quickswap_v3->ramses_v3","loan_amount_raw":amount,"error_type":type(exc).__name__,"error":str(exc),"retryable":_retryable_failure(exc)})
                    try:
                        reverse.append(build_ramses_v3_to_quickswap_v3_route(
                            rpc, quickswap, ramses, amount_in=amount, token_a=USDC_E, token_b=token_b,
                            ramses_tick_spacing=tick_spacing, block=context))
                    except Exception as exc:
                        failures.append({"direction":"ramses_v3->quickswap_v3","loan_amount_raw":amount,"error_type":type(exc).__name__,"error":str(exc),"retryable":_retryable_failure(exc)})
                ...
                tiles.append({
                    ...
                    "quickswap_v3_pool":quick_pool,"status":"SUCCESS","coverage_status":coverage_status,
                    "expected_direction_evaluations":expected,"accounted_direction_evaluations":accounted,
                ...
                coverage_status=classify_s10_tile_failure(exc)
                tiles.append({
    ...
    coverage = summarize_s10_coverage(
        tiles,
