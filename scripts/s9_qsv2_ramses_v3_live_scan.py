#!/usr/bin/env python3
"""Read-only live hunt for QuickSwap V2 <-> Ramses V3 on Polygon."""
...
def _scan_endpoint(endpoint: str) -> dict[str, Any]:
    http = PolygonRPCHTTPTransport(
        PolygonRPCHTTPConfig(
            provider_name=f"qsv2-ramses-v3:{endpoint}",
...
def main() -> int:
    ...
    artifact = {
        "schema_version": 1,
        "mission": "PHANTOMX QS V2 <-> RAMSES V3 READ-ONLY LIVE SCAN",
        "strategy": "S9-QSV2-RAMSES-V3",
        "read_only": True,
        "signing": False,
        "submission": False,
        "broadcast": False,
