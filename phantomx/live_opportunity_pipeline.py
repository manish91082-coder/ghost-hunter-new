"""Compose concrete live discovery through proven execution preflight.

This module provides the deterministic orchestration seams before the signer
boundary. It performs no signing, submission, broadcast, or live-capital
operation.
"""
...
def discover_prepare_and_preflight_best_opportunity(
    ...
    *,
    ...
    """Run live discovery, economic execution assembly, then EVM preflight.

    The returned preflight result is the final deterministic artifact before the
    signer boundary. Any discovery, economic, assembly, or preflight failure is
    collapsed into one fail-closed pipeline error and no signing/submission occurs.
    """
    try:
