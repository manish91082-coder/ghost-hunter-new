...
def executor_topology_hash(*, asset: str, token_mid: str, first_on_quickswap: bool, uniswap_fee: int, aave_pool: str, quickswap_router: str, uniswap_v3_router: str) -> str:
    ...
    payload = b"".join((_word_address(asset, "asset"), _word_address(token_mid, "token_mid"), _word_bool(first_on_quickswap, "first_on_quickswap"), _word_uint(uniswap_fee, "uniswap_fee", max_value=0xFFFFFF), _word_address(aave_pool, "aave_pool"), _word_address(quickswap_router, "quickswap_router"), _word_address(uniswap_v3_router, "uniswap_v3_router")))
...
def build_executor_transaction(intent: ExecutionIntent, *, token_mid: str, first_on_quickswap: bool, uniswap_fee: int, amount_out_min_first: int, amount_out_min_second: int, minimum_surplus: int, aave_pool: str, quickswap_router: str, uniswap_v3_router: str, gas_limit: int, max_fee_per_gas: int, max_priority_fee_per_gas: int) -> BoundExecutorCall:
    ...
    topology_hash = executor_topology_hash(asset=intent.loan_asset, token_mid=token_mid, first_on_quickswap=first_on_quickswap, uniswap_fee=uniswap_fee, aave_pool=aave_pool, quickswap_router=quickswap_router, uniswap_v3_router=uniswap_v3_router)
